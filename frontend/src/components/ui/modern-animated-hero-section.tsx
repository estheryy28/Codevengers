...
// Modified to accept custom phrases and hide title if not needed
        ...
        if (mounted && scramblerRef.current) {
            ...
            const next = () => {
                if (scramblerRef.current) {
                    scramblerRef.current.setText(defaultPhrases[counter]).then(() => {
                        setTimeout(next, 2000)
                    })
                    counter = (counter + 1) % defaultPhrases.length
                }
            }
...
const RainingLetters: React.FC<RainingLettersProps> = ({ showTitle = false, phrases, children }) => {
    ...
    return (
        <div className="relative w-full min-h-screen bg-black overflow-x-hidden">
            ...
            {/* Children content overlay - Changed to relative to allow scrolling */}
            {children && (
