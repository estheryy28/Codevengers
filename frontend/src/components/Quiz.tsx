...
export default function Quiz() {
    ...
    // Cheating detection
    // Cheating detection
    // No local state needed for warning modal anymore as we logout immediately
    ...
    useEffect(() => {
        ...
        const enterFullscreen = async () => {
            if (!document.fullscreenElement) {
                try {
                    ...
                    console.error("Fullscreen blocked", error);
                }
            }
        };
        ...
        const handleViolation = async (message: string) => {
            // Check if exam is still active (e.g. not finishing/logging out)
            if (!isExamActive.current) return;

            // Disable further checks to prevent duplicate calls
            isExamActive.current = false;

            console.log("Detecting violation:", message);

            try {
                // Call backend to record violation
                // IMPORTANT: Ensure this request completes before we log the user out
                const res = await axios.post('/auth/violation');
                const { violations, isDisqualified } = res.data.data;

                // Only logout after the request is successful
                logout();

                if (isDisqualified || violations >= 3) {
                    navigate('/login', { state: { message: `Exam terminated: ${message}. You have been disqualified for multiple cheating attempts.` } });
                } else {
                    const remaining = 3 - violations;
                    navigate('/login', { state: { message: `Exam terminated: ${message}. Violation ${violations}/3 recorded. You have ${remaining} attempt(s) remaining.` } });
                }
            } catch (error) {
                console.error("Failed to record violation", error);

                // Even if the request fails, we must logout for security
                logout();
                navigate('/login', { state: { message: `Exam terminated: ${message}. Security violation detected.` } });
            }
        };

        const handleVisibilityChange = () => {
            if (document.hidden && isExamActive.current) {
                handleViolation('You switched tabs');
            }
        };

        const handleFullscreenChange = () => {
            if (!document.fullscreenElement && isExamActive.current) {
        ...
        const handleWindowBlur = () => {
            if (isExamActive.current) {
                handleViolation('You switched to another application');
            }
        };


        const handleKeyDown = (e: KeyboardEvent) => {
            // Block F12, Ctrl+Shift+I, Ctrl+Shift+J, Ctrl+Shift+C, Ctrl+U, Escape, Alt+Tab (if possible)
            if (
                e.key === 'F12' ||
                e.key === 'Escape' ||
                ...
                // Trigger violation for Escape key immediately
                if (e.key === 'Escape') {
                    handleViolation('You pressed the Escape key');
                }
            ...
            mutations.forEach((mutation) => {
                mutation.addedNodes.forEach((node) => {
                    if (node.nodeType === 1) {
                        ...
                        if (element.id !== 'root' && element.tagName !== 'SCRIPT' && element.tagName !== 'STYLE' && element.tagName !== 'LINK' && !element.classList.contains('swal2-container')) { // Allow sweetalert if used, but generic check
                            // element.remove(); 
                            // Commenting out aggressive removal for now to prevent issues with modals or other portals if any
                            // If aggressive security is needed, we can uncomment, but ensure our own Modals are not removed.
                            // Our React Portals (like the Warning Modal) might be attached to body.
                        }
                    }
                });
            });
        ...
        // CLEANER INTERVAL - REDUCED AGGRESSION
        // We need to be careful not to delete our own React Portals if they mount outside root.
        // Assuming standard React app where everything is in #root.
        // If we use a Portal for the modal, it might be an issue.
        // For this specific 'showWarning' modal, I'll put it INSIDE the standard render flow (inside #root), so it's safe.

        // 3. Block interaction with anything outside root
        const handleGlobalClick = (e: MouseEvent) => {
            const root = document.getElementById('root');
            // Allow clicks on the Modal if it ends up outside root (e.g. Radix UI primitives), but here we will render it inline.
            // If inline, it is inside root.
        ...
        const fetchChallenges = async () => {
            try {
                ...
                if (user?.isDisqualified) {
                    // Disable protections before logout
                    isExamActive.current = false;
                    logout();
                    navigate('/login', { state: { message: 'You have been disqualified for multiple cheating attempts.' } });
                }
            } catch (err) {
    ...
    return (
        <RainingLetters>
            <div className="min-h-screen pt-20 pb-10 px-4">
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="flex items-center justify-between mb-6 animate-fade-in-up">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <button
                                onClick={async () => {
                                    // Disable exam active check before opening confirm dialog
                                    // Confirm dialog causes window blur which triggers violation
                                    isExamActive.current = false;

                                    // Wait a tick to ensure state updates (though ref is sync)
                                    if (window.confirm('Are you sure you want to finish the quiz? This will submit your final score.')) {
                                        ...
                                        // Re-enable exam active check if user cancelled
                                        // And ensure we are in fullscreen
                                        try {
                                            if (!document.fullscreenElement) {
                                                await document.documentElement.requestFullscreen();
                                            }
                ...
                {/* Warning Modal Removed - Immediate Logout Implemented */}
