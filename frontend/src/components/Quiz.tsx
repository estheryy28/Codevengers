...
import { CheckCircle2, XCircle, Play, Lightbulb, ChevronLeft, ChevronRight, AlertTriangle, Clock } from 'lucide-react';
                                ...
                                {result && (
                                    <div className={`card border ${getStatusColor(result.status)}`}>
                                        ...
                                        {!result.compilationResult.success && (
                                            <div className="mb-4 p-3 bg-red-900/30 rounded-lg font-mono text-sm text-red-300 overflow-x-auto">
                                                {result.compilationResult.errors.map((err, i) => (
                                                    <div key={i}>{err}</div>
                                                ))}
                                            </div>
                                        )}
